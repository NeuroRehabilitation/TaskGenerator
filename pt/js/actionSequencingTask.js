function actionSequencingTask(type, size, newpage) {
  ...
  var implicit = [
    "Ficar transpirado depois de uma corrida.",
  ...
  var instructions =
    "Por favor, indique a ordem pela qual os passos que se seguem\n devem ser executados.";
  doc.setFontSize(15);
  ...
  if (type == 0) {
    for (
      i = index * 6;
      i < index * 6 + 6;
      i++ //There are 6 steps per action
    ) {
    ...
    for (
      i = 0;
      i < size; //There are 6 steps per action
    ...
    doc.text(30, 105, "Ação: " + explicit[index * 7]);
    for (
      i = index * 7 + 1;
      i < index * 7 + 7;
      i++ //There are 6 steps per action
