...
function associationTask(size,newpage)
{
    ...
    var instructions = "Por favor, indique, com uma linha, quais elementos da esquerda \nfazem par com os da direita.";
    doc.setFontSize(15);
	...
	while (size > 0)
	{
		...
		for (i=0;i<size_x;i=i+1)
		{
			...
			doc.addImage(left[index], 'JPEG', 25, 80 + i*2*dimension/1.7, dimension, dimension);
			doc.roundedRect(25,80 + i*2*dimension/1.7,dimension,dimension,3,3);
			doc.roundedRect(25+dimension-3,80 + i*2*dimension/1.7+dimension/2-3,6,6,3,3,'F');
			left.splice(index, 1);
			
			index = Math.floor(Math.random()*right.length);
			doc.addImage(right[index], 'JPEG', 188 - dimension, 80 + i*2*dimension/1.7, dimension, dimension);	
			doc.roundedRect(188 - dimension, 80 + i*2*dimension/1.7, dimension, dimension,3,3);		
			doc.roundedRect(188 - dimension -3,80 + i*2*dimension/1.7+dimension/2-3,6,6,3,3,'F');			
