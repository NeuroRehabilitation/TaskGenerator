function categorizationModel(categories, elements)
 {
    ...
    // Attention 
    var attention = (-3.26 + categories*3.75 + categories*elements*(-0.41)).toFixed(1);
...
function categorizationTask(categories,elements,newpage)
{
    ...
    var instructions = "Por favor, agrupe as imagens abaixo por categorias de pertença.";
    doc.setFontSize(15);
	...
	for (i=0;i<Math.max(elements,categories);i=i+1)
	{
		for (j=0;j<Math.min(elements,categories);j=j+1)
		{
			...
			doc.addImage(pairs[index], 'JPEG', 25 + ((170-dimension*Math.min(elements,categories))/Math.min(elements,categories)+dimension)*j, 80 + i*2*dimension/1.7, dimension, dimension);
			
			doc.roundedRect(25 + ((170-dimension*Math.min(elements,categories))/Math.min(elements,categories)+dimension)*j, 80 + i*2*dimension/1.7 , dimension, 1.1*dimension,2,2);
			
			doc.line(25 + ((170-dimension*Math.min(elements,categories))/Math.min(elements,categories)+dimension)*j+0.2*dimension, 80 + i*2*dimension/1.7 + 1.05*dimension,25 + ((170-dimension*Math.min(elements,categories))/Math.min(elements,categories)+dimension)*j + dimension*.8, 80 + i*2*dimension/1.7 + 1.05*dimension);
	...
	doc.setFontSize(8);
    doc.text(20, 275, "* Imagens do banco de dados IPNP nomeação de figuras (http://crl.ucsd.edu/experiments/ipnp/method/getpics/getpics.html)");
