...
function mazeModel(elements_param)
{
    ...
    var exfunctions = (2.390 + elements*1.375).toFixed(1);
...
var instructions = "Por favor, desenhe o caminho desde a entrada até à saída neste \nlabirinto.";
...
doc.text(position_x-7, position_y-20,'Comece aqui!');
doc.text(position_x+3, position_y-10,'x');

while(i < labirynth.length)
{
	if (labirynth[i]=='-')
		doc.line(position_x+k*width,position_y+j*height,position_x+k*width+width,position_y+j*height);
	if (labirynth[i]=='+')
	{
		if (i>1)
			if ((labirynth[i-1]!=' ')&(labirynth[i-1]!='.'))
				doc.line(position_x+k*width,position_y+j*height,position_x+k*width+width/2,position_y+j*height);
		
		if ((labirynth[i+1]!=' ')&(labirynth[i+1]!='.'))
			doc.line(position_x+k*width+width/2,position_y+j*height,position_x+k*width+width,position_y+j*height);
		
	}
	
	if (labirynth[i]=='|')
	{		
		doc.line(position_x+k*width+width/2,position_y+(j-1)*height,position_x+k*width+width/2,position_y+j*height+height);
	}
