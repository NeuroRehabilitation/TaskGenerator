function sequencingModel(size,step,order,where,missing)
{
    ...
    if (where == 1)
        position = (size-missing)/2;
	
    // Ex. Funct.
    var exfunctions = (6.682 + missing*(-0.014) + position*(-0.002)).toFixed(1);

    // Attention 
    var attention = (6.923 + missing*(-0.020) + position*(-0.003)).toFixed(1);

    // Memory
    var memory = (5.364 + missing*(-0.027) + position*(-0.003)).toFixed(1);
    
    // Language
    var language = (4.722 + missing*(-0.020) + position*(-0.003)).toFixed(1);
    
    // Difficulty
    var difficulty = (1.290 + step*1.232 + AscendentYes*(-0.841)).toFixed(1);
...
doc = createFrame(doc, page,sequencingModel(size,step,order,where,missing));

var instructions = "Por favor, preencha os números em falta nas seguintes sequências.";
doc.setFontSize(15);
...
for (j=0;j<9;j++)
{
    ...
    if (step < 0)
        first += size * Math.abs(step);
    
    for (i=0;i<size;i++)
        sequence.push((first+step*i).toString());
        
    var position = 0;

    if (where == 1)
        position = Math.floor(Math.random()*(size-missing+1));

    for (i=0;i<missing;i++)
        sequence[position+i] = "__";
