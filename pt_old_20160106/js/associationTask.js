...
function associationTask(size,newpage)
{
	...
	while (size > 0)
	{
		...
		for (i=0;i<size_x;i=i+1)
		{
			...
			doc.addImage(left[index], 'JPEG', 35, 80 + i*2*dimension/1.7, dimension, dimension);
			left.splice(index, 1);
			
			index = Math.floor(Math.random()*right.length);
			doc.addImage(right[index], 'JPEG', 100+7*step, 80 + i*2*dimension/1.7, dimension, dimension);		
